...
class HMLC(nn.Module):
    ...
    def forward(self, features, labels):
        labels = labels.cpu()  # torch.Size([16, 637])
        features = features.cpu()  # torch.Size([16, 768])
        ...
        cumulative_loss = self.sup_con_loss(features, mask=mask_labels)
        cumulative_loss = self.layer_penalty(torch.tensor(1 / self.config.alpha).type(torch.float)) * cumulative_loss
